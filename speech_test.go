...
func TestSpeech_StartTasks(t *testing.T) {
	opts := make([]communicate.Option, 0)
	opts = append(opts, communicate.WithVoice("zh-CN-YunxiaNeural"))
...
func TestSpeech_StartTasksWithOptionsToZip(t *testing.T) {
	...
	options := make(map[string][]communicate.Option)
	options["test.mp3"] = []communicate.Option{communicate.WithVoice("zh-CN-YunyangNeural")}
